...
bool ColladaDocument::parseFile( const string &fileName )
{
	...
	map<string, DaeNode*> nodeMap;

	for( unsigned int i = 0; i < libVisScenes.visScenes.size(); ++i )
 	{
 		for( unsigned int j = 0; j < libVisScenes.visScenes[i]->nodes.size(); ++j )
 		{
			nodeMap["#"+libVisScenes.visScenes[i]->nodes[j]->id] = libVisScenes.visScenes[i]->nodes[j];
		}
	}
	...
	if( !node1.isEmpty() )
	{
		...
		if( !node2.isEmpty() )
		{
			...
			if( !node2.getChildNode( "instance_visual_scene" ).isEmpty() )
 				log( "Warning: Found more than one scene instance; ignoring following instances" );
 		}
 	}
