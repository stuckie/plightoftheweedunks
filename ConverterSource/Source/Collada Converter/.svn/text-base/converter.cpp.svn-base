...
Converter::~Converter()
{
	...
	for( unsigned int i = 0; i < _cameras.size(); ++i ) delete _cameras[i];
}
								   ...
								   Matrix4f transAccum, vector< Matrix4f > animTransAccum )
{
	...
	{
		DaeCamera* dae_camera = doc.libCameras.findCamera(node.instances[0].url);
		if( dae_camera )
		{
			Camera* the_camera = new Camera(dae_camera);
			_cameras.push_back( the_camera );
			oNode = the_camera;
	...
	{
			DaeLight* dae_light = doc.libLights.findLight(node.instances[0].url);
			if( dae_light )
 			{
				Light* the_light = new Light(dae_light);
				_lights.push_back( the_light );
				oNode = the_light;
 				oNode->daeInstance = &node.instances[0];
 			}
	}
...
void Converter::writeLights( ColladaDocument &doc,const string &name )
{
	...
	for( unsigned int i = 0; i < _lights.size(); ++i )
	{
		...
		if (light)
		{
			...
			rot.y = radToDeg( rot.y );
			rot.z = radToDeg( rot.z );
			outf << "\t" << light->name << " = Horde:AddLightNode( 0, \"" << light->name << "\", LightMat, \"LIGHTING\", \"SHADOWMAP\" );"  << endl;
			outf << "\tHorde:SetNodeTransform( " << light->name << ", " << trans.x << ", " << trans.y << ", " << trans.z << ", " << rot.x << ", " << rot.y << ", " << rot.z << ", " << scale.x << ", " << scale.y << ", " << scale.z << " );" << endl;
			if(light->point)
			{
				outf << "\tHorde:SetNodeParamFloat( " << light->name << ", \"FOV\", 360);" << endl;
				outf << "\tHorde:SetNodeParamFloat( " << light->name << ", \"Radius\", 100 );" << endl;
			}
			if(light->spot)
			{
				outf << "\tHorde:SetNodeParamFloat( " << light->name << ", \"FOV\", " << light->outer_cone << " );" << endl;
				outf << "\tHorde:SetNodeParamFloat( " << light->name << ", \"Radius\", 400 );" << endl;
			}
			outf << "\tHorde:SetNodeParamInt( " << light->name << ", \"ShadowMapCount\", 3 );" << endl;
			outf << "\tHorde:SetNodeParamFloat( " << light->name << ", \"ShadowSplitLambda\", 0.9 );" << endl;
			outf << "\tHorde:SetNodeParamFloat( " << light->name << ", \"ShadowMapBias\", 0.001 );" << endl;
			outf << "\tHorde:SetNodeParamFloat( " << light->name << ", \"Col_R\", " << light->r * light->intensity << " );" << endl;
			outf << "\tHorde:SetNodeParamFloat( " << light->name << ", \"Col_G\", " << light->g * light->intensity << " );" << endl;
			outf << "\tHorde:SetNodeParamFloat( " << light->name << ", \"Col_B\", " << light->b * light->intensity << " );" << endl;
			outf << endl;
		}
	}
...
void Converter::writeCameras( ColladaDocument &doc,const string &name )
{
	...
	for( unsigned int i = 0; i < _cameras.size(); ++i )
	{
		...
		if (camera)
		{
			...
			rot.y = radToDeg( rot.y );
			rot.z = radToDeg( rot.z );
			outf << "\t" << camera->name << " = Horde:AddCameraNode( 0, \"" << camera->name << "\", PipeRes );" << endl;
			outf << "\tHorde:SetupCameraView( " << camera->name << ", " << camera->yfov << ", " << camera->aspect << ", " << camera->znear << ", " << camera->zfar << ");" << endl;
			outf << "\tHorde:SetNodeTransform( " << camera->name << ", " << trans.x << ", " << trans.y << ", " << trans.z << ", " << rot.x << ", " << rot.y << ", " << rot.z << ", " << scale.x << ", " << scale.y << ", " << scale.z << " );" << endl;
			outf << endl;
		}
	}
...
void Converter::writeLevel( ColladaDocument &doc,const string &name )
{
	...
	for( unsigned int i = 0; i < _meshes.size(); ++i )
		if (!_meshes[i]->daeNode->physicsMaterial.empty())
			if ((strcmp( _meshes[i]->daeNode->physicsObjectType.c_str(), "moving" ) == 0) || (strcmp( _meshes[i]->daeNode->physicsObjectType.c_str(), "scripted" ) == 0))
				outf << "dofile (\"data/scripts/" << folder << "/physics/scripted/" << _meshes[i]->daeNode->name << ".physics.lua\");" << endl;
	...
	for( unsigned int i = 0; i < _meshes.size(); ++i )
		if (!_meshes[i]->daeNode->targetable.empty())
		{
			...
			outf << "\tTARGETS = { next = TARGETS,  x = " << trans.x << ", y = " << trans.y << ", z = " << trans.z << ", name = \"" << _meshes[i]->daeNode->name << "\" };" << endl;
		}
	outf << "end" << endl;
	outf.close();
	...
	for( unsigned int i = 0; i < _meshes.size(); ++i )
		if (!_meshes[i]->daeNode->waypointType.empty())
		{
			...
			//outf << "\tWAYPOINTS = { next = WAYPOINTS,  x = " << trans.x << ", y = " << trans.y << ", z = " << trans.z << ", name = \"" << _meshes[i]->daeNode->name << "\", type = " << _meshes[i]->daeNode->waypointType << ", amount = " << _meshes[i]->daeNode->waypointAmount << " };" << endl;
			outf << "\tAI:AddWayPoint( " << _meshes[i]->daeNode->waypointType << ", \"" << _meshes[i]->daeNode->name << "\", " << trans.x << ", " << trans.y << ", " << trans.z << " ); " << endl;
		}
	outf << "end" << endl;
	outf.close();
	...
	for( unsigned int i = 0; i < _meshes.size(); ++i )
		if (!_meshes[i]->daeNode->SpawnDelay.empty())
		{
			...
			outf << "\tSPAWNERS = { next = SPAWNERS, x = " << trans.x << ", y = " << trans.y + scale.y/2 + 5 << ", z = " << trans.z << ", numofbert = " << _meshes[i]->daeNode->NumberofBert << ", numofernie = " << _meshes[i]->daeNode->NumberofErnie << ", timer = 0, bert = 0, ernie = 0, delay = " << _meshes[i]->daeNode->SpawnDelay << ", name = \"" << _meshes[i]->daeNode->name << "\" };" << endl;
		}
	outf << "end" << endl;
	outf.close();
...
void Converter::writePhysics(ColladaDocument &doc, const string &name )
{
	...
	for( unsigned int i = 0; i < _meshes.size(); ++i )
	{
		...
		if (strcmp( _meshes[i]->daeNode->physicsObjectType.c_str(), "static" ) == 0)
		{
			...
			outf << "\t" << _meshes[i]->daeNode->name << "_static = ODE:CreateStaticBox( " << scale.x << ", " << scale.y << ", " << scale.z << ", " << trans.x << ", " << trans.y << ", " << trans.z << ", " << rot.x << ", " << rot.y << ", " << rot.z << ", \"" << _meshes[i]->daeNode->physicsMaterial << "\" );" << endl;
			outf << endl;
		}
	}
	...
	for( unsigned int i = 0; i < _meshes.size(); ++i )
	{
		...
		if (strcmp( _meshes[i]->daeNode->physicsObjectType.c_str(), "dynamic" ) == 0)
		{
			...
			outf << "\t" << _meshes[i]->daeNode->name << "_dynamic = ODE:CreateDynamicBox( " << scale.x << ", " << scale.y << ", " << scale.z << ", " << trans.x << ", " << trans.y << ", " << trans.z << ", " << rot.x << ", " << rot.y << ", " << rot.z << ", 10, \"" << _meshes[i]->daeNode->physicsMaterial << "\" );" << endl;
			outf << "\tHorde:FindNodes( LevelMod, \"" << _meshes[i]->daeNode->name << "\", \"Mesh\" );" << endl;
			outf << "\tnodeHandle = Horde:GetNodeFindResult(0);" << endl;
			outf << "\t" << name << "_DYNAMIC = { next = " << name << "_DYNAMIC, node = nodeHandle, physics = " << _meshes[i]->daeNode->name << "_dynamic };" << endl;
			outf << endl;
		}
	}
	...
	for( unsigned int i = 0; i < _meshes.size(); ++i )
	{
		if (!_meshes[i]->daeNode->physicsMaterial.empty())
		{
			...
			if ((strcmp( _meshes[i]->daeNode->physicsObjectType.c_str(), "moving" ) == 0) || (strcmp( _meshes[i]->daeNode->physicsObjectType.c_str(), "scripted" ) == 0))
			{
				...
				outf << "\t" << _meshes[i]->daeNode->name << "_scripted = ODE:CreateMovingBox( " << scale.x << ", " << scale.y << ", " << scale.z << ", " << trans.x << ", " << trans.y << ", " << trans.z << ", " << rot.x << ", " << rot.y << ", " << rot.z << ", \"" << _meshes[i]->daeNode->physicsMaterial << "\" );" << endl;
				outf << "\tHorde:FindNodes( LevelMod, \"" << _meshes[i]->daeNode->name << "\", \"Mesh\" );" << endl;
				outf << "\tnodeHandle = Horde:GetNodeFindResult(0);" << endl;
				outf << "\t" << name << "_SCRIPTED = { next = " << name << "_SCRIPTED, node = nodeHandle, physics = " << _meshes[i]->daeNode->name << "_scripted };" << endl;
