...
int SDLGLRender::CreateRenderer ( void )
{
    ...
    {
        SGZLogger.warn("Couldn't init SDLGL Renderer.\n");
        return -1;
    }
	else SGZLogger.debug("SDLGL Renderer initialised!\n");
...
int SDLGLRender::SGZCreateWindow( std::string title, int WindowWidth, int WindowHeight, int WindowBPP )
{
...
int SDLGLRender::SGZCloseWindow( void )
{
    /** Close The Window - We can't DO this in SDL until the end! */
    return 0;
};

int SDLGLRender::SetFullScreen( bool set )
{
